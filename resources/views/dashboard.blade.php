    ...
    <div class="p-gutter max-w-container-max mx-auto w-full space-y-8">
        ...
        <div class="flex justify-between items-end">
            <div>
                ...
                <p class="font-body-md text-body-md text-on-surface-variant">System status is operational. {{ $summary['low_stock_count'] }} items require attention.</p>
            </div>
            <div class="flex gap-3">
                <button class="bg-surface-container-lowest border border-outline-variant px-4 py-2 rounded-lg font-label-md text-label-md flex items-center gap-2 hover:bg-surface-container-low transition-all">
            ...
            <div class="lg:col-span-2 bg-surface-container-lowest p-6 rounded-xl border border-outline-variant shadow-sm flex flex-col">
                ...
                <div class="flex-1 min-h-[300px] w-full relative flex items-end gap-2">
                    ...
                    <svg class="absolute bottom-0 left-0 w-full h-[260px] overflow-visible pointer-events-none" viewbox="0 0 1000 300">
                        ...
                        <path d="M0,250 Q50,220 100,230 T200,180 T300,210 T400,140 T500,160 T600,100 T700,120 T800,60 T900,80 T1000,40" fill="none" stroke="#3525cd" stroke-width="3"></path>
                        <path d="M0,250 Q50,220 100,230 T200,180 T300,210 T400,140 T500,160 T600,100 T700,120 T800,60 T900,80 T1000,40 V300 H0 Z" fill="url(#chartGradient)"></path>
                    </svg>
                    ...
                    <div class="space-y-4">
                            ...
                            @foreach($summary['expiring_soon_products'] as $product)
                                <div class="bg-surface-container-lowest p-4 rounded-lg border border-error/20 shadow-sm relative">
                                    ...
                                    <p class="font-body-md text-body-md text-on-surface leading-tight">
                                        <strong>{{ $product->name }}</strong> expires soon on <span class="text-error font-semibold">{{ \Carbon\Carbon::parse($product->expiration_date)->format('Y-m-d') }}</span>.
                                    </p>
                                </div>
                            @endforeach
                        @else
                            <div class="bg-surface-container-lowest p-4 rounded-lg border border-outline-variant shadow-sm text-center py-6">
                        ...
                        @if(isset($summary['top_selling_products']) && count($summary['top_selling_products']) > 0)
                            <div class="bg-surface-container-lowest p-4 rounded-lg border border-primary/20 shadow-sm">
                                ...
                                <p class="font-body-md text-body-md text-on-surface leading-tight">
                                    <strong>{{ $summary['top_selling_products'][0]->name ?? 'N/A' }}</strong> is your highest-selling product this period. Ensure stable supply chains.
                                </p>
                            </div>
                        @endif
                    </div>
            ...
            <div class="lg:col-span-2 bg-surface-container-lowest rounded-xl border border-outline-variant shadow-sm overflow-hidden flex flex-col justify-between">
                <div>
                    <div class="p-6 border-b border-outline-variant flex justify-between items-center">
                        ...
                        <span class="px-2.5 py-0.5 rounded-full text-xs font-semibold bg-amber-100 text-amber-800">Action Required</span>
                    </div>
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            <thead class="bg-surface-container-low border-b border-outline-variant">
                                <tr>
                                    <th class="px-6 py-4 font-label-md text-label-md text-on-surface-variant uppercase">Product Name</th>
                                    <th class="px-6 py-4 font-label-md text-label-md text-on-surface-variant uppercase">SKU</th>
                                    <th class="px-6 py-4 font-label-md text-label-md text-on-surface-variant uppercase">Category</th>
                                    <th class="px-6 py-4 font-label-md text-label-md text-on-surface-variant uppercase">Current Stock</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-outline-variant">
                                @forelse($summary['low_stock_products'] as $product)
                                    <tr class="hover:bg-surface-container-low/50 transition-colors">
                                        <td class="px-6 py-4 font-body-md text-body-md font-medium text-on-surface">{{ $product->name }}</td>
                                        <td class="px-6 py-4 font-mono-sm text-mono-sm text-on-surface-variant">{{ $product->sku }}</td>
                                        <td class="px-6 py-4 font-body-md text-body-md text-on-surface-variant">{{ $product->category->name ?? '-' }}</td>
                                        <td class="px-6 py-4 font-body-md text-body-md">
                                            <span class="text-error font-semibold">{{ $product->stock_quantity }} Units</span>
                                        </td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="4" class="text-center py-8 text-on-surface-variant italic">No low stock items detected. Awesome!</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
                ...
                <div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="w-full text-left border-collapse">
                            <thead class="bg-surface-container-low border-b border-outline-variant">
                                <tr>
                                    <th class="px-6 py-4 font-label-md text-label-md text-on-surface-variant uppercase">Product</th>
                                    <th class="px-6 py-4 font-label-md text-label-md text-on-surface-variant uppercase text-right">Units Sold</th>
                                </tr>
                            </thead>
                            <tbody class="divide-y divide-outline-variant">
                                @forelse($summary['top_selling_products'] as $product)
                                    <tr class="hover:bg-surface-container-low/50 transition-colors">
                                        <td class="px-6 py-4 font-body-md text-body-md font-medium text-on-surface">{{ $product->name }}</td>
                                        <td class="px-6 py-4 font-body-md text-body-md text-right font-semibold text-primary">{{ $product->sold_quantity ?? 0 }}</td>
                                    </tr>
                                @empty
                                    <tr>
                                        <td colspan="2" class="text-center py-8 text-on-surface-variant italic">No sales recordings found.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
