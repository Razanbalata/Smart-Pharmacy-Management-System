...
<div class="max-w-4xl mx-auto p-6 space-y-6">
    <div>
        ...
        <p class="text-sm text-on-surface-variant dark:text-gray-400 mt-1">Register a new medication or medical item into the inventory.</p>
    </div>
        ...
        <form method="POST" action="{{ route('products.store') }}" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div class="space-y-1.5">
                        ...
                        <input type="number" step="0.01" id="purchase_price" name="purchase_price" value="{{ old('purchase_price') }}" placeholder="0.00" class="w-full px-4 py-2.5 rounded-xl border border-outline-variant/60 bg-transparent text-on-surface dark:text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all" required>
                    </div>

                    <div class="space-y-1.5">
                        <label for="selling_price" class="text-sm font-medium text-on-surface dark:text-gray-300">Sale Price ($) *</label>
                        <input type="number" step="0.01" id="selling_price" name="selling_price" value="{{ old('selling_price') }}" placeholder="0.00" class="w-full px-4 py-2.5 rounded-xl border border-outline-variant/60 bg-transparent text-on-surface dark:text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all" required>
                    </div>

                    <div class="space-y-1.5">
                        <label for="stock_quantity" class="text-sm font-medium text-on-surface dark:text-gray-300">Initial Stock *</label>
                        <input type="number" id="stock_quantity" name="stock_quantity" value="{{ old('stock_quantity', 0) }}" class="w-full px-4 py-2.5 rounded-xl border border-outline-variant/60 bg-transparent text-on-surface dark:text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all" required>
                    </div>

                    <div class="space-y-1.5">
                        <label for="minimum_stock" class="text-sm font-medium text-on-surface dark:text-gray-300">Alert Min Stock *</label>
                        <input type="number" id="minimum_stock" name="minimum_stock" value="{{ old('minimum_stock', 10) }}" class="w-full px-4 py-2.5 rounded-xl border border-outline-variant/60 bg-transparent text-on-surface dark:text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all" required>
                    </div>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="space-y-1.5">
                    ...
                    <input type="date" id="expiration_date" name="expiration_date" value="{{ old('expiration_date') }}" class="w-full px-4 py-2.5 rounded-xl border border-outline-variant/60 bg-transparent text-on-surface dark:text-white focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all">
                </div>

                <div class="space-y-1.5">
                    <label for="batch_number" class="text-sm font-medium text-on-surface dark:text-gray-300">Batch Number (Lot #)</label>
                    <input type="text" id="batch_number" name="batch_number" value="{{ old('batch_number') }}" placeholder="e.g., B-LOT102" class="w-full px-4 py-2.5 rounded-xl border border-outline-variant/60 bg-transparent text-on-surface dark:text-white placeholder:text-on-surface-variant/50 focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary transition-all">
                </div>
            </div>
